...
class Word(BaseModel):
    ...
    def from_char_dicts(
        ...
        tokens = list(Word._build_tokens(text, char_dicts, writing_mode, tokenizer, begin_of_word))
        bbox = join_rects(*(t.bbox for t in tokens))
    ...
    def _build_tokens(
        ...
        token_groups = Word._build_token_strings_with_groups_of_escaped_tokens(token_ids, tokenizer, begin_of_word)
        for i, (t, b) in zip(token_ids, Word._build_token_boxes(token_groups, text, char_dicts, writing_mode)):
    ...
    def _build_token_strings_with_groups_of_escaped_tokens(
        ...
        tokens = Word._remove_begin_of_word_markers(tokens, begin_of_word)
        token_groups = Word._group_consecutive_escaped_tokens(tokens)
        return token_groups
    ...
    def _group_consecutive_escaped_tokens(tokens: Iterable[str]) -> Iterable[List[str]]:
        return (
            ...
            for _, g in groupby(
                enumerate(tokens), key=lambda idx_tok: -1 if Word._is_escaped(idx_tok[1]) else idx_tok[0]
            )
        )
    ...
    def _build_token_boxes(
        ...
        for tg in token_groups:
            result_iter, start_idx = Word._create_token_boxes_for_token_group(
    ...
    def _create_token_boxes_for_token_group(
        ...
        if len(token_group) == 1 and not Word._is_escaped(token_group[0]):
            detok_text = token_group[0]
            token_box, start_idx = Word._extract_token_box(detok_text, original_text, char_dicts, start_idx)
            return [(detok_text, token_box)], start_idx
        else:
            token_boxes, start_idx = Word._create_boxes_for_escaped_tokens(
    ...
    def _create_boxes_for_escaped_tokens(
        ...
        token_bytes = "".join(map(Word._extract_bytes, tokens))
        detok_text = bytes.fromhex(token_bytes).decode("utf-8")
        token_box, start_idx = Word._extract_token_box(detok_text, original_text, char_dicts, start_idx)
        token_boxes = Word._split_token_box(token_box, len(tokens), writing_mode)
    ...
    def _extract_token_box(
        ...
        token_box = join_rects(*(c["bbox"] for c in char_dicts[start_idx:end_idx]))
...
class Line(BaseModel):
    ...
    def from_line_dict(
        ...
        words = map(
            partial(Word.from_char_dicts, writing_mode=writing_mode, tokenizer=tokenizer, begin_of_word=begin_of_word),
            split_chars,
        )
        return Line(
            text=text,
...
class Block(BaseModel):
    ...
    def from_block_dict(
        ...
        if block_dict["type"] != 0:
            return None
        lines = map(partial(Line.from_line_dict, tokenizer=tokenizer, begin_of_word=begin_of_word), block_dict["lines"])
...
class Page(BaseModel):
    ...
    def from_page_dict(
        ...
        blocks = [
            block
            for block in map(
                partial(Block.from_block_dict, tokenizer=tokenizer, begin_of_word=begin_of_word), page_dict["blocks"]
            )
...
class Document(BaseModel):
    ...
    @staticmethod
    def from_pdf_path(pdf_path: str, tokenizer: PreTrainedTokenizer, begin_of_word: str = "▁") -> Document:
        doc = fitz.open(pdf_path)
        pages = map(
            partial(Page.from_page_dict, tokenizer=tokenizer, begin_of_word=begin_of_word),
...
if __name__ == "__main__":
    ...
    tokenizer_name_or_path = "VAGOsolutions/SauerkrautLM-7b-v1" if len(sys.argv) < 4 else sys.argv[3]
    tokenizer = LlamaTokenizer.from_pretrained(tokenizer_name_or_path, legacy=True, add_bos_token=False)

    doc = Document.from_pdf_path(pdf_path, tokenizer)
