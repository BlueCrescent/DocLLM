...
def test_output_shape_is_input_shape(config: DocLLMLlamaConfig, input_sizes: InputSizes, model_inputs: ModelInputs):
    ...
    output, _, _, _ = attention(model_inputs.input_embeddings, model_inputs.spatial_embeddings)
    assert output.shape == model_inputs.input_embeddings.shape


def test_with_attention_mask(config: DocLLMLlamaConfig, model_inputs: ModelInputs, attention_mask: torch.Tensor):
    attention = DocLLMAttention(config)
    output, _, _, _ = attention(
        model_inputs.input_embeddings, model_inputs.spatial_embeddings, attention_mask=attention_mask
    )
    assert output.shape == model_inputs.input_embeddings.shape
...
def test_with_cache(
    config: DocLLMLlamaConfig, model_inputs: ModelInputs, past_key_value: Cache, spatial_past_key_value: Cache
):
